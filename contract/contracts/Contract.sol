...
contract RegistryLand {
    ...
    function addLand(uint _area, string memory _address, uint _landPrice,string memory _allLatiLongi, uint _propertyPID,string memory _surveyNum, string memory _document) public {
        ...
        landsCount++;
lands[landsCount] = Landreg(landsCount, _area, _address, _landPrice,_allLatiLongi,_propertyPID, _surveyNum , _document,false,payable(msg.sender),false);        MyLands[msg.sender].push(landsCount);
    ...
    function requestforBuy(uint _landId) public
    {
        ...
        requestCount++;
LandRequestMapping[requestCount] = LandRequest(requestCount, payable(lands[_landId].ownerAddress), payable(msg.sender), _landId, reqStatus.requested, false);
